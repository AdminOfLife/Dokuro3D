#include "SpecularModelSmoothDemo.hpp"
using namespace dkr;

SpecularModelSmoothDemo::SpecularModelSmoothDemo() {
	rotation = 0.0f;
}

// Initialization and unloading
void SpecularModelSmoothDemo::Init(dkr::GraphicsMemory* _memory, dkr::Transformer* _transformer, dkr::IRenderTarget* _renderer) {
...
void SpecularModelSmoothDemo::Unload(dkr::GraphicsMemory* _memory, dkr::Transformer* _transformer, dkr::IRenderTarget* _renderer) {
	_memory->RemoveBuffer(cubeId);
	_transformer->GetLightInfo()->Clear();
}

// Update and draw the demo.
void SpecularModelSmoothDemo::Update(float time, dkr::GraphicsMemory* _memory, dkr::Transformer* _transformer, dkr::IRenderTarget* _renderer) {
	rotation += time * 1.0f;
}
void SpecularModelSmoothDemo::Draw(HWND hWnd, dkr::GraphicsMemory* _memory, dkr::Transformer* _transformer, dkr::IRenderTarget* _renderer) {
...
float SpecularModelSmoothDemo::Expires() {
	return 10.0f;
}